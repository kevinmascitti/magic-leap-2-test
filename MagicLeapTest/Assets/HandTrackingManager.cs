...
{
    [SerializeField] private GameObject handPrefabForKeypoint;
    
    [SerializeField] [Range(0,1.0f)] private float boneVisibilityConfidence;
    ...
    void Start()
    {
        if (MagicLeap.Android.Permissions.CheckPermission(MagicLeap.Android.Permissions.HandTracking))
        {
        ...
        {
            Logger.Instance.LogError($"MLPermission for hand tracking was not granted");
        }
    }
    ...
    private void Update()
    {
        ...
        {
            Logger.Instance.LogError(leftHandDevice.name + " device detected");
            DisplayFeatures($"{leftHandDevice.name}", leftHandDevice);
            BuildHandSimpleSkeleton(HandSkeletonFor.LeftHand);
        ...
        {
            Logger.Instance.LogError(leftHandDevice.name + " device detected");
            DisplayFeatures($"{rightHandDevice.name}", rightHandDevice);
            BuildHandSimpleSkeleton(HandSkeletonFor.RightHand);
    ...
    private void DisplayFeatures(string handDeviceName, InputDevice device)
    {
        ...
        if(device.TryGetFeatureUsages(features) && verboseHandTrackingLog)
        {
            foreach (var feature in features)
            {
                Logger.Instance.LogInfo($"Display Features - {handDeviceName}: {feature.name}");
            }
        }
    }
    ...
    private void DisplayGesturesInfo(HandSkeletonFor handSkeletonFor)
    {
        ...
        InputSubsystem.Extensions.MLGestureClassification.PostureType featurePostureType = (InputSubsystem.Extensions.MLGestureClassification.PostureType)gestureInt;
        Logger.Instance.LogInfo($"Gestures - {handSkeletonFor} gesture: " + featurePostureType);
        // Mention that you can also get additional finger data including angles with MLGestureClassification API
    }

    private void BuildHandSimpleSkeleton(HandSkeletonFor handSkeletonFor)
    {
        ...
        Logger.Instance.LogError(device.name + " " + confidence);
        if (confidence >= boneVisibilityConfidence)
        {
            Logger.Instance.LogWarning(CommonUsages.handData.ToString());
            if (device.TryGetFeatureValue(CommonUsages.handData, out Hand hand))
            {
                Logger.Instance.LogWarning("ENTER");
    ...
    //     InputSubsystem.Extensions.MLGestureClassification.StopTracking();
    // }
}